...
export const useSportHistoryStore = create<SportsStore>((set, get) => ({
  sportHistory: [
    {
      id: '1',
      sportCategory: ['跑步'],
  ...
  // 添加运动记录
  addSportRecord: (item) => set((state) => ({
    sportHistory: [...state.sportHistory, item]
  })),
  
  // 更新运动记录
  updateSportRecord: (id, updates) => set((state) => ({
  ...
  // 删除运动记录
  deleteSportRecord: (id) => set((state) => ({
  ...
  getTotalDuration: () => {
    const { sportHistory } = get()
    return sportHistory.reduce((total, record) => total + record.duration, 0)
  }
}))